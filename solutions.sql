...
--- code on how to find quarterly revenue for 2023
SELECT
...
total_current_customersORDER BY units_sold DESC;
...
SELECT 
ci.city_name,
ROUND(AVG(s.total),2) AS average_sales_amount
FROM customers AS c
JOIN sales as s
ON
...
ORDER BY city_name, revenue DESC;
...
9. **Monthly Sales Growth**  Sales growth rate: Calculate the percentage growth (or decline) in sales over different time periods (monthly).
